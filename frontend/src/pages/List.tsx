...
/**
 * Construit l'URL complète pour une ressource de stockage (image) de l'hôtel.
 * Remplace VITE_STORAGE_BASE_URL par VITE_API_BASE_URL/storage/ si vous utilisiez l'autre méthode
 * ou utilise VITE_STORAGE_BASE_URL directement selon votre backend Laravel/S3/etc.
 */
const getStorageUrl = (photoPath: string | null): string | null => {
  if (!photoPath) return null;
  // J'utilise ici VITE_STORAGE_BASE_URL comme dans votre code initial
  return `${import.meta.env.VITE_STORAGE_BASE_URL}/${photoPath}`;
  
  // Si votre configuration est du type "URL de l'API + /storage/chemin_du_fichier", utilisez plutôt:
  // return `${import.meta.env.VITE_API_BASE_URL}/storage/${photoPath}`; 
};

// ------------------- COMPOSANT D'AFFICHAGE D'IMAGE (MIS À JOUR) -------------------

const HotelImage: React.FC<{ hotel: Hotel }> = ({ hotel }) => {
  const [imageError, setImageError] = useState(false);
  
  // Utilisation de la fonction centralisée
  const imageUrl = getStorageUrl(hotel.photo);

  useEffect(() => {
    // Réinitialise l'état de l'erreur si l'URL de la photo change
    setImageError(false);
  }, [hotel.photo]);
...
const List: React.FC = () => {
  ...
  const handleOpenPopup = (hotel?: Hotel) => {
    if (hotel) {
      ...
      // Utilisation de la fonction centralisée
      setPhotoPreview(getStorageUrl(hotel.photo));
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    // ... Logique handleFormSubmit (identique) ...
    e.preventDefault();
    setLoading(true);
    setError("");
          ...
          {filteredHotels.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredHotels.map((hotel) => (
                ...
                >
                  {/* Appel au composant d'image mis à jour */}
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  ...
                  {/* Aperçu de la photo (MISE À JOUR) */}
